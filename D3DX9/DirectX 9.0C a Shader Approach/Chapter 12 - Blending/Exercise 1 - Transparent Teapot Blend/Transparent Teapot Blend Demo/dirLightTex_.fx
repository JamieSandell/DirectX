...
float4 DirLightTexPS(float2 tex0 : TEXCOORD0, float3 posW : TEXCOORD1, float3 normalW : TEXCOORD2) : COLOR
{
	...
	diffuseAmbient.rgb = ambient + diffuse;
	diffuseAmbient.a   = gDiffuseMtrl.a;
	float4 spec4 = float4(spec, 0.0f);
	//=======================================================

	//float3 texColor = tex2D(TexS, tex0).rgb;
	//diffuse = diffuseAmbient.rgb * texColor;
    //return float4(diffuse + spec4.rgb, diffuseAmbient.a);
	return float4(((diffuse + ambient)*tex2D(TexS, tex0).rgb) + spec, gDiffuseMtrl.a);
}
