...
void Camera::update(float dt)
{
	...
	// Rotate camera's look and up vectors around the camera's up vector (yaw)
	D3DXMatrixRotationAxis(&R, &mUpW, yaw);
	D3DXVec3TransformCoord(&mLookW, &mLookW, &R);
	D3DXVec3TransformCoord(&mUpW, &mUpW, &R);

	// Rotate the camera's right and up vectors around the camera's look vector (roll)
	D3DXMatrixRotationAxis(&R, &mLookW, roll);
	D3DXVec3TransformCoord(&mRightW, &mRightW, &R);
	D3DXVec3TransformCoord(&mUpW, &mUpW, &R);
