...
Camera::Camera()
{
	...
	//Jump code
	mAccel = D3DXVECTOR3(0.0f, -9.8f, 0.0f); //acceleration due to gravity.
	mVel = D3DXVECTOR3(0.0f, 2.0f, 0.0f); //velocity, only used for jumping.
	mCurrentTime = 0.0f; //incremented each frame with dt
	mJumpStartTime = 0.0f;
	mCurrentlyJumping = false; //used to allow only one execution of "jump" until the jump has been completed.
}
...
void Camera::update(float dt, Terrain* terrain, float offsetHeight)
{
	...
	{
		if (mCurrentlyJumping == false)
		{
			mCurrentlyJumping = true;
			mJumpStartTime = mCurrentTime;
		}
	}

	// Constant acceleration.
	if (mCurrentlyJumping)
	{
		float t = mCurrentTime - mJumpStartTime;
		mPosW.y = mPosW.y + mVel.y*t + 0.5f * mAccel.y * t * t;
	}
	...
	if(terrain != 0)
	{
		...
		else
		{
			...
			if (newPos.y-offsetHeight > mPosW.y) //Falling through the terrain
			{
				// Now the difference of the new position and old (current) 
				// position approximates a tangent vector on the terrain.
				D3DXVECTOR3 tangent = newPos - mPosW;
