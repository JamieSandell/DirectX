		...
		float yOffset)
{
	...
	// Default fog variable values
	mFogColor = D3DXVECTOR3(0.5f, 0.5f, 0.5f); //Grayish
	mFogStart = 1.0f;
	mFogRange = 250.0f;
...
void Terrain::buildEffect()
{
	...
	setFogStart(mFogStart);
}

void Terrain::setFogColor(const D3DXVECTOR3& col)
{
	HR(mFX->SetValue(mhFogColor, &col, sizeof(D3DXVECTOR3)));
	mFogColor = col;
}

void Terrain::setFogRange(float range)
{
	if (range >= 0.0f)
		mFogRange = range;
	else
		mFogRange = 0.0f;
	HR(mFX->SetFloat(mhFogRange, mFogRange));
}

void Terrain::setFogStart(float start)
{
	if (start >= 0.0f)
		mFogStart = start;
	else
		mFogStart = 0.0f;
	HR(mFX->SetFloat(mhFogStart, mFogStart));
}

float Terrain::getFogRange()
{
	return mFogRange;
}

float Terrain::getFogStart()
{
	return mFogStart;
}