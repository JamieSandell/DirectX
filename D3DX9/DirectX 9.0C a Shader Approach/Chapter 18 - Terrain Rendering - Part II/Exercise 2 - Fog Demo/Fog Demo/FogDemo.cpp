//=============================================================================
// FogDemo.cpp. Modified from Frank D Luna's Props Demo.
//
...
//			 Use '9/0' to decrease/increase Fog Start.
//			 Use '-/=' to decrease/increase Fog Range.
//=============================================================================
	...
	PSTR cmdLine, int showCmd)
{
	...
	FogDemo app(hInstance, "Fog Demo", D3DDEVTYPE_HAL, D3DCREATE_HARDWARE_VERTEXPROCESSING);
	gd3dApp = &app;
...
FogDemo::FogDemo(HINSTANCE hInstance, std::string winCaption, D3DDEVTYPE devType, DWORD requestedVP)
...
void FogDemo::updateScene(float dt)
{
	mTime += dt;
	...
	if( gDInput->keyDown(DIK_M) )
		mFreeCamera = true;
	if (gDInput->keyDown(DIK_MINUS)) //Decrease fog range
		mTerrain->setFogRange(mTerrain->getFogRange()-100.0f*dt);
	if (gDInput->keyDown(DIK_EQUALS)) //Increase fog range
		mTerrain->setFogRange(mTerrain->getFogRange()+100.0f*dt);
	if (gDInput->keyDown(DIK_9)) //Decrease fog start
		mTerrain->setFogStart(mTerrain->getFogStart()-100.0f*dt);
	if (gDInput->keyDown(DIK_0)) //Increase fog start
		mTerrain->setFogStart(mTerrain->getFogStart()+100.0f*dt);
...
void FogDemo::drawObject(Object3D& obj, const D3DXMATRIX& toWorld)
{
...
void FogDemo::buildGrassFin(GrassVertex* v, WORD* k, int& indexOffset, 
...
void FogDemo::drawText()
{
	...
	sprintf(buffer, "Fog Start:\t%.2f%\n"
		"Fog Range:\t%.2f%", mTerrain->getFogStart(), mTerrain->getFogRange());
	R.left = md3dPP.BackBufferWidth-150; R.top = 5; R.bottom = 0; R.right = 0;
	HR(mFont->DrawText(0, buffer, -1, &R, DT_NOCLIP, D3DCOLOR_XRGB(0,0,0)));
}