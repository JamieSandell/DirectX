...
#include "ClippedPlanarShadowDemo.h"
	...
	PSTR cmdLine, int showCmd)
{
	...
	ClippedPlanarShadowDemo app(hInstance, "Clipped Planar Shadow Demo", D3DDEVTYPE_HAL, D3DCREATE_HARDWARE_VERTEXPROCESSING);
	gd3dApp = &app;
...
ClippedPlanarShadowDemo::ClippedPlanarShadowDemo(HINSTANCE hInstance, std::string winCaption, D3DDEVTYPE devType, DWORD requestedVP)
...
ClippedPlanarShadowDemo::~ClippedPlanarShadowDemo()
{
	delete mGfxStats;
	ReleaseCOM(mRoomVB);
	ReleaseCOM(mTeapot);
...
bool ClippedPlanarShadowDemo::checkDeviceCaps()
{
	D3DCAPS9 caps;
...
void ClippedPlanarShadowDemo::onLostDevice()
{
	mGfxStats->onLostDevice();
	HR(mFX->OnLostDevice());
}

void ClippedPlanarShadowDemo::onResetDevice()
{
...
void ClippedPlanarShadowDemo::updateScene(float dt)
{
	mGfxStats->update(dt);
...
void ClippedPlanarShadowDemo::drawScene()
{
...
void ClippedPlanarShadowDemo::buildRoomGeometry()
{
...
void ClippedPlanarShadowDemo::buildFX()
{
...
void ClippedPlanarShadowDemo::buildViewMtx()
{
...
void ClippedPlanarShadowDemo::buildProjMtx()
{
...
void ClippedPlanarShadowDemo::drawRoom()
{
...
void ClippedPlanarShadowDemo::drawMirror()
{
	HR(mFX->SetMatrix(mhWVP, &(mRoomWorld*mView*mProj)));
	D3DXMATRIX worldInvTrans;
	D3DXMatrixInverse(&worldInvTrans, 0, &mRoomWorld);
...
void ClippedPlanarShadowDemo::drawTeapot()
{
...
void ClippedPlanarShadowDemo::drawReflectedTeapot()
{
...
void ClippedPlanarShadowDemo::clipShadow()
{
	HR(gd3dDevice->SetRenderState(D3DRS_STENCILENABLE,    true));
	HR(gd3dDevice->SetRenderState(D3DRS_STENCILFUNC,      D3DCMP_ALWAYS));
	HR(gd3dDevice->SetRenderState(D3DRS_STENCILREF,       0x1));
...
void ClippedPlanarShadowDemo::drawTeapotShadow()
{
	HR(gd3dDevice->SetRenderState(D3DRS_STENCILENABLE,    true));
	HR(gd3dDevice->SetRenderState(D3DRS_STENCILFUNC,      D3DCMP_EQUAL));
	HR(gd3dDevice->SetRenderState(D3DRS_STENCILREF,       0x1));
	...
	// Position shadow.
	D3DXVECTOR4 lightDirection(mLightVecW, 0.0f);
...
void ClippedPlanarShadowDemo::genSphericalTexCoords()
{
