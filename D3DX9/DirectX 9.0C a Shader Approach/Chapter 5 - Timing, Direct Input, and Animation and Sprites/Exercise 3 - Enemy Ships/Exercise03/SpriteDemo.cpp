//////////////////////////////////////////////////////////////////////////
//Create or find some free images of alien spaceships. Modify these
//images by inserting an alpha channel and marking the pixels that are to
//be displayed. Then modify the Sprite demo by using these modified images
//to include enemy ship sprites.
//////////////////////////////////////////////////////////////////////////
...
private:
	...
	IDirect3DTexture9* mEnemyTex;
	D3DXVECTOR3 mEnemyCenter;
...
: D3DApp(hInstance, winCaption, devType, requestedVP), BULLET_SPEED(2500.0f), MAX_SHIP_SPEED(1500.0f),
	SHIP_ACCEL(1000.0f), SHIP_DRAG(0.85f), ZOOM_SPEED(1000.0f)
{
	if(!checkDeviceCaps())
	{
	...
	HR(D3DXCreateTextureFromFile(gd3dDevice, "mySprite.png", &mEnemyTex));
	...
	mEnemyCenter = D3DXVECTOR3(64.0f, 64.0f, 64.0f);

	mShipPos      = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	mShipSpeed    = 0.0f;
	mShipRotation = 0.0f;

	mEnemyPos = D3DXVECTOR3(100.0f, 0.0f, 0.0f);
...
void SpriteDemo::onResetDevice()
{
	...
	D3DXMATRIX V;
	mCamera.pos = D3DXVECTOR3(0.0f, 0.0f, -5000.0f);
	mCamera.up = D3DXVECTOR3(0.0f, 1.0f, 0.0f);
	mCamera.target = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	D3DXMatrixLookAtLH(&V, &mCamera.pos, &mCamera.target, &mCamera.up);
...
void SpriteDemo::updateCamera(float dt)
{
	mCamera.pos.z -= gDInput->mouseDY() * ZOOM_SPEED * dt;
...
void SpriteDemo::drawEnemy()
{
	// Turn on the alpha test
	HR(gd3dDevice->SetRenderState(D3DRS_ALPHATESTENABLE, true));

	D3DXMATRIX T;
	D3DXMatrixTranslation(&T, mEnemyPos.x, mEnemyPos.y, mEnemyPos.z);
	HR(mSprite->SetTransform(&T));

	// Draw the enemy
	HR(mSprite->Draw(mEnemyTex, 0, &mEnemyCenter, 0, D3DCOLOR_XRGB(255, 255, 255)));
	HR(mSprite->Flush());
