...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd )
{
	...
	Exercise01 app(hInstance, "Exercise01", D3DDEVTYPE_HAL, D3DCREATE_HARDWARE_VERTEXPROCESSING);
	gd3dApp = &app;
...
void Exercise01::drawScene()
{
	...
	for (UINT i = 0; i < numPasses; ++i)
	{
		...
		HR(mFX->SetMatrix(mhWVP, &(mView*mProjection)));
...
void Exercise01::drawCube(void)
{
	HR(gd3dDevice->SetRenderState(D3DRS_FILLMODE, D3DFILL_WIREFRAME));
	HR(gd3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, 8, 0, 12));
}

void Exercise01::buildGeoBuffers()
{
	//Obtain a pointer to a new vertex buffer
	HR(gd3dDevice->CreateVertexBuffer(8 * sizeof(VertexPos), D3DUSAGE_WRITEONLY, 0, D3DPOOL_MANAGED, &mVB, 0));

	//Now lock it to obtain a pointer to its internal data, and write the cube's vertex data.
...
void Exercise01::buildFX()
{
	//Create the FX from a .fx file.
	ID3DXBuffer* errors = 0;
	HR(D3DXCreateEffectFromFile(gd3dDevice, "transform.fx", 0, 0, D3DXSHADER_DEBUG, 0, &mFX, &errors));
...
void Exercise01::buildProjectionMatrix()
{
	...
	D3DXMatrixPerspectiveFovLH(&mProjection, D3DX_PI * 0.25f, w/h, 1.0f, 5000.0f);
}