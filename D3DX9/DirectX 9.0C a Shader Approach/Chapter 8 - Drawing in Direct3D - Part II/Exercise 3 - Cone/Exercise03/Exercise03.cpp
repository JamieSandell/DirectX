	...
	PSTR cmdLine, int showCmd)
{
	...
	Exercise03 app(hInstance, "Exercise03", D3DDEVTYPE_HAL, D3DCREATE_HARDWARE_VERTEXPROCESSING);
	gd3dApp = &app;
...
Exercise03::Exercise03(HINSTANCE hInstance, std::string winCaption, D3DDEVTYPE devType, DWORD requestedVP)
	: D3DApp(hInstance, winCaption, devType, requestedVP)
{
	...
	HR(D3DXCreateCylinder(gd3dDevice, 0.0f, 1.0f, 6.0f, 20, 20, &mCone, 0));

	buildGeoBuffers();
	buildFX();

	int numConeVerts    = mCone->GetNumVertices() * 14;
	int numConeTris     = mCone->GetNumFaces()    * 14;

	mGfxStats->addVertices(mNumGridVertices);
	mGfxStats->addVertices(numConeVerts);
	mGfxStats->addTriangles(mNumGridTriangles);
	mGfxStats->addTriangles(numConeTris);
...
bool Exercise03::checkDeviceCaps()
{
	D3DCAPS9 caps;
...
void Exercise03::onLostDevice()
{
	mGfxStats->onLostDevice();
	HR(mFX->OnLostDevice());
}

void Exercise03::onResetDevice()
{
...
void Exercise03::updateScene(float dt)
{
	mGfxStats->update(dt);
...
void Exercise03::buildGeoBuffers()
{
