	...
	PSTR cmdLine, int showCmd)
{
	...
	Exercise05 app(hInstance, "Exercise05", D3DDEVTYPE_HAL, D3DCREATE_HARDWARE_VERTEXPROCESSING);
	gd3dApp = &app;
	...
	: D3DApp(hInstance, winCaption, devType, requestedVP)
{
	...
	//HR(D3DXCreateCylinder(gd3dDevice, 0.0f, 1.0f, 6.0f, 20, 20, &mTeapot, 0));
	//HR(D3DXCreateTeapot(gd3dDevice, &mTorus, 0));
	HR(D3DXCreateTorus(gd3dDevice, 0.1f, 1.0f, 3, 30, &mTorus, NULL))

	buildGeoBuffers();
	buildFX();

	int numTorusVerts    = mTorus->GetNumVertices() * 14;
	int numTorusTris     = mTorus->GetNumFaces()    * 14;

	mGfxStats->addVertices(mNumGridVertices);
	mGfxStats->addVertices(numTorusVerts);
	mGfxStats->addTriangles(mNumGridTriangles);
	mGfxStats->addTriangles(numTorusTris);
...
bool Exercise05::checkDeviceCaps()
{
	D3DCAPS9 caps;
...
void Exercise05::updateScene(float dt)
{
	mGfxStats->update(dt);
...
void Exercise05::buildGeoBuffers()
{
