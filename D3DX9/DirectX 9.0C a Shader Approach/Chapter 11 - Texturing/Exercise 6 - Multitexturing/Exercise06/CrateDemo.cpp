//=============================================================================
// Demonstrates multi-texturing a box.
// both textures rotate, default values mean one texture rotates faster than
// the other.
//
...
private:
	...
	IDirect3DTexture9*     mFlareTex;
	IDirect3DTexture9*	   mFlareAlphaTex;
...
: D3DApp(hInstance, winCaption, devType, requestedVP)
{
	...
	HR(D3DXCreateTextureFromFile(gd3dDevice, "flare.bmp", &mFlareTex));
	HR(D3DXCreateTextureFromFile(gd3dDevice, "flarealpha.bmp", &mFlareAlphaTex));
...
CrateDemo::~CrateDemo()
{
	...
	ReleaseCOM(mBoxVB);
	ReleaseCOM(mBoxIB);
	ReleaseCOM(mFlareTex);
	ReleaseCOM(mFlareAlphaTex);
	ReleaseCOM(mFX);
...
void CrateDemo::drawScene()
{
	...
	HR(mFX->SetTexture(mhTex1, mFlareTex));
	HR(mFX->SetTexture(mhTex2, mFlareAlphaTex));
	HR(mFX->SetFloat(mhTime, mTime));
	HR(mFX->SetFloat(mhSpeed, 5.0f));
	HR(mFX->SetFloat(mhSpeed2, 2.0f));
...
void CrateDemo::buildFX()
{
	...
	mhSpeed			= mFX->GetParameterByName(0, "gSpeed");
	mhSpeed2			= mFX->GetParameterByName(0, "gSpeed2");
}
